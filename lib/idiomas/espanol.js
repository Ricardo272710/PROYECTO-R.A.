...
const smsRowner = () => { return `*Este Comando Solo Lo Puedo Usar Yo Y Mi Creador RICARDO-ANTHONY 🕷️*` }//NUMERO DE BOT
const smsOwner = () => { return `*Este Comando Solo Lo Puedo Usar Yo Y Mi Creador RICARDO-ANTHONY 🕷️🕷️*` }//OWNER
const smsMods = () => { return `*Este Comando Solo Lo Puedo Usar Yo , Mi Creador Y Los Mods Del Bot 🕷️*` }//MODERADORES
const smsPremium = () => { return `*Necesitas Ser Usuario Premium 🕷️*` }//USUARIOS PREMIUM
const smsGroup = () => { return `*Comando Solamente Permitido Para El Uso En Grupos 🕷️*` }//PARA GRUPOS
const smsPrivate = () => { return `*Este Comando Solo Se Puede Usar En El Privado Del Bot 🕷️*` }//AL PRIVADO
const smsAdmin = () => { return `*Necesitas Rogarle A Un Admin Para Que Puedas Usar Esto 🕷️` }//ADMINS
const smsBotAdmin = () => { return `Necesito Ser Admin Antes 🕷️*` }//BOT CON ADMIN
const smsUnreg = () => { return `*🕷️ No Te Encuentras Registrado 🕷️*\n*Debes Registrarte Con La Palabra .Reg Seguido De Tu Nombre Y Tu Edad*\n*Ejemplo :*\n*.Reg Yallico.18*` }//VERIFICAR
const smsRestrict = () => { return `*Hablale A Mk Creador Para Que Active Este Comando 🕷️*` }//RESTRINGIR COMANDO

//MENU LISTA
const smsTime = () => { return `𝙏𝙞𝙚𝙢𝙥𝙤 𝘼𝙘𝙩𝙪𝙖𝙡`}
const smsUptime = () => { return `𝙏𝙞𝙚𝙢𝙥𝙤 𝙖𝙘𝙩𝙞𝙫𝙤`}
const smsVersion = () => { return `𝙑𝙚𝙧𝙨𝙞𝙤𝙣 𝙙𝙚𝙡 𝙋𝙍𝙊𝙔𝙀𝘾𝙏𝙊-𝙈𝘿`}
...
//_allantilink.js
const smsTextoYT = () => { return '𝙋𝙍𝙊𝙔𝙀𝘾𝙏𝙊 𝙓 🕷️ '} 
...
const eGataCoins = () => { return '🪙 ProyectoCoins' }
const eGataTickers = () => { return '🎫 Proyecto Tickers' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar a ProyectoBot`}
